--------------------------------------------------------
--  DDL for Procedure SP01001_5320
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "SCVENTASTIENDA"."SP01001_5320" 
...
Descripcion: Recibe el registro 5320 en texto separados por comas, 
             actualiza la tabla TAPEDIDOS (FIPEDSTAT y FNPEDSALDO)
...
BEGIN
   ...
   MERGE INTO SCVENTASTIENDA.TAPEDIDOS DATOS
       USING (SELECT pa_FiPaisId PAIS, 
                     pa_FiCanalId CANAL, 
                     pa_FiSucursalId  SUC, 
                     recTAEST01001_5320.FINOPEDIDO PEDIDO
                FROM DUAL) VALORES
        ...
        WHEN MATCHED THEN
             UPDATE
                SET  FIPEDSTAT = recTAEST01001_5320.FIPEDSTAT,
                     FNPEDSALDO = csl_ValorCero;
   ...
   MERGE INTO SCVENTASTIENDA.TACREDITO DATOS
       USING (SELECT pa_FiPaisId PAIS, 
        ...
        WHEN MATCHED THEN
             UPDATE
                SET  FICLASIFCUENTA = recTAEST01001_5320.FICLASIFCUENTA;
...
/
  ...
  GRANT EXECUTE ON "SCVENTASTIENDA"."SP01001_5320" TO "USRVENTAST";
  GRANT EXECUTE ON "SCVENTASTIENDA"."SP01001_5320" TO "SCACTUALIZADORVT";
